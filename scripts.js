...
function drawBarChart() {
    ...
    svg.selectAll("mybar")
        ...
        .join("text")
        .attr('class', 'val')
        .attr('transform', d => `translate(${x(d.location) + 10},
        ${y(d.number) - 15}), rotate(-90)`)
        .text(function(d) {
            return d.number;  // Value of the text
        });
...
function drawLocationChart(town) {
    ...
    svg.selectAll("mybar")
        ...
        .join("text")
        .attr('class', 'val')
        // let dd = d3.quantile(x(d.flat_type),0)
        .attr('transform', d => `translate(${x(d.flat_type)+x.bandwidth()/2},
        ${y(d.number) - 15}), rotate(-90)`)
        .text(function(d) {
            return d.number;  // Value of the text
        });
    ...
    d3.select('#chart')
        ...
        .attr('id', 'back-button')
        .attr('class', 'button')
        .attr("transform", `translate(${backgroundWidth-margin.right-80}, ${margin.top+578.5})`)
    ...
    d3.select('#chart')
        ...
        .attr("transform", `translate(${backgroundWidth-margin.right-50}, ${margin.top+602})`)
    ...
    var color_model = d3.rgb(15, 182, 85)
    // Model button
    d3.select('#chart')
        .append('rect')
        .attr('id', 'test-button')
        .attr('class', 'button')
        .attr("transform", `translate(${backgroundWidth-margin.right-230}, ${margin.top+578.5})`)
        .on('mouseover', buttonOnMouseOver)
        .on('mouseout', buttonOnMouseOut)
        .on('click', modelButtonCallback)
        .attr('width', 60)
        .attr('height', 40)
        .attr('fill', color_model);
    d3.select('#chart')
        .append('text')
        .attr('id', 'test-button-text')
        .attr('class', 'button-text')
        .attr("transform", `translate(${backgroundWidth-margin.right-200}, ${margin.top+602})`)
        .style("text-anchor", "middle")
        .on('mouseover', textOnMouseOver)
        .on('mouseout', textOnMouseOut)
        .on('click', modelButtonCallback)
...
function drawLocationChart2(town) {
    ...
    svg.selectAll("mybar")
        ...
        .attr('class', 'val')
        // let dd = d3.quantile(x(d.flat_type),0)
        .attr('transform', d => `translate(${x(d.flat_model)+x.bandwidth()/2},
        ${y(d.number) - 15}), rotate(-90)`)
        .text(function(d) {
            return d.number;  // Value of the text
        });
    ...
    var color_type = d3.rgb(28, 124, 202)
    // Type Button
    d3.select('#chart')
        .append('rect')
        .attr('id', 'test-button')
